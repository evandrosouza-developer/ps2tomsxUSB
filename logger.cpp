...
static void semihost_write(uint32_t fh, const unsigned char *buffer, unsigned int length)
{
...
#ifdef SERIAL_DEBUG
	uint32_t i;
	uint16_t qty_in_serial_tx_ring;
	fh &= 0xFFFFFFFF;
	//Send data via serial, instead of through semihost
	i = 0;
	while( (buffer[i] != 0) && (i < length) )
	{
		while( (qty_in_serial_tx_ring = ring_put_ch(&serial_tx_ring, buffer[i])) == 0 )
			__asm("nop");			//while serial buffer is full, wait for room
		i++;
		if(qty_in_serial_tx_ring == 1)
		{
...
void debug_flush(void) {
    ...
    debugBuffer[0] = 0;     //Here we clean the debugBuffer
}
...
void debug_begin(uint16_t bps) {
    //  TODO
    bps &= 0xFFFF;          //Only to avoid warning: unused parameter 'bps' [-Wunused-parameter]
}
